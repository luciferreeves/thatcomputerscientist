    ...
    if request.user.is_authenticated and (
        ...
        if q:
            ...
            return render(
                request,
                "blog_admin/posts.html",
                {"title": 'Search Results for "{}"'.format(q), "posts": posts},
            )
        else:
...
def new_post(request):
    if request.user.is_authenticated and (
        ...
        if request.method == "POST":
            ...
            post_image = (
                request.FILES["post_image"] if "post_image" in request.FILES else None
            )
            additional_tags = (
                request.POST.get("additional_tags")
                if request.POST.get("additional_tags")
                else ""
            )
            post_date = (
                request.POST.get("post_date")
            ...
            if request.POST.get("post_id"):
                ...
                try:
                    ...
                    post.slug = slug
                    post.tags.set(
                        [Tag.objects.get_or_create(name=tag.strip())[0] for tag in tags]
                    )
                    post.date = post_date
                    if post_image:
                    ...
                    messages.error(
                        request,
                        "Error while updating the post: {}".format(e),
                        extra_tags="new_post_message",
                    )
                ...
                if not title or not category or not tags or not post_image:
                    ...
                    return_object = {
                        ...
                        "tags_value": tags,
                        "slug_value": slug,
                        "post_image_value": post_image,
                        "all_tags": all_tags,
                        "post_date_value": post_date,
                    }
                    return render(
                        request,
                        "blog_admin/new_post.html",
                        {
                            "title": "New Post",
                    ...
                    try:
                        ...
                        post.save()
                        return redirect(
                            reverse("blog-admin:edit-post", kwargs={"slug": post.slug})
                        )
                    except Exception as e:
                        messages.error(
                            request,
                            "Some error occured while creating post.",
                            extra_tags="error",
                        )
                        return redirect("blog-admin:posts")
    ...
    if request.user.is_authenticated and (
        ...
        if request.method == "POST":
            ...
            try:
                ...
                messages.error(
                    request, "Error: {}".format(e), extra_tags="edit_post_create_error"
                )
                return render(
                    request,
                    "blog_admin/edit_post.html",
